...
class App:
    ...
    def listar_chaves(self, filtro=None):
        ...
        ttk.Label(top, text="Select a key to delete or view details:").pack()
        for chave in chaves:
    ...
    def generate_and_show_keys(self):
        ...
        try:
            ...
            self.show_keys_ui(priv_key, pub_key, senha, salt)
        except Exception as e:
            self.show_error(f"Falha em gerar chaves: {str(e)}")

    def show_keys_ui(self, priv_key, pub_key, senha, salt):
        key_window = tk.Toplevel(self.root)
        key_window.title("Chaves Geradas")
        key_window.geometry("700x750")
    ...
    def importar_chave_privada_interactive(self):
        ...
        if private_key_path:
            senha = self.ask_for_password("Digite uma senha para encriptar a chave privada:")
            try:
                importar_chave_privada(private_key_path, senha)
